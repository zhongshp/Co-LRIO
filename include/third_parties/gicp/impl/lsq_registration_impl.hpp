...
void LsqRegistration<PointTarget, PointSource>::computeTransformation(PointCloudSource& output, const Matrix4& guess) {
  ...
  for (int i = 0; i < max_iterations_ && !converged_; i++) {
    // auto begin_iter_time = chrono::high_resolution_clock::now();
    nr_iterations_ = i;
    ...
    // auto end_iter_time = chrono::high_resolution_clock::now();
    // auto iter_duration = chrono::duration_cast<chrono::microseconds>(end_iter_time - begin_iter_time).count();
    // std::cout << "iterations_ " << i << " " << float(iter_duration)/1e3 << std::endl;
  }
